...
function resources_deployment(){
    ...
    if [ "$DB" == "mysql" ]
    then
        helm install wso2ei-rdbms-service -f $deploymentRepositoryLocation/deploymentRepository/helm_ei/mysql/values.yaml stable/mysql
        sleep 30s
    fi
    if [ "$DB" == "postgres" ]
    then
        helm install wso2ei-rdbms-service -f $deploymentRepositoryLocation/deploymentRepository/helm_ei/postgresql/values.yaml stable/postgresql
        sleep 30s
    fi
    if [ "$DB" == "mssql" ]
    then
        helm install wso2ei-rdbms-service -f $deploymentRepositoryLocation/deploymentRepository/helm_ei/mssql/values.yaml stable/mssql-linux
        kubectl create -f $deploymentRepositoryLocation/deploymentRepository/helm_ei/jobs/db_provisioner_job.yaml
        sleep 30s
    fi
